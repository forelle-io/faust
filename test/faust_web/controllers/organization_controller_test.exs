defmodule FaustWeb.OrhanizationControllerTest do
  ...
  describe "index" do
    ...
    test "страница списка всех организация, когда пользователь авторизован", %{conn: conn} do
      ...
      conn =
        conn
        |> authorize_conn(current_organization)
  ...
  describe "show" do
    ...
    test "страница другогой организации, когда текущий пользователь авторизован", %{conn: conn} do
      current_organization = organization_fixture()
      other_organization = other_organization_fixture()

      conn =
        conn
        |> authorize_conn(current_organization)
        |> get(Routes.organization_path(conn, :show, other_organization))
